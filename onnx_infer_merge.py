...
def get_plate_result(img, session_rec):  # 识别后处理
    ...
    # print("Shape of y_onnx_plate:", y_onnx_plate.shape)
    # print("Values of y_onnx_plate:", y_onnx_plate)
    print("Shape of y_onnx_color:", y_onnx_color.shape)
    print("Values of y_onnx_color:", y_onnx_color)
...
def get_plate_result2(img, session_rec):  # 识别后处理
    img = rec_pre_precessing(img)
    y_onnx_plate, y_onnx_color = session_rec.run([session_rec.get_outputs()[0].name, session_rec.get_outputs()[1].name],
                                                 {session_rec.get_inputs()[0].name: img})
    # print("Shape of y_onnx_plate:", y_onnx_plate.shape)
    ...
    # 对字符预测应用softmax
    plate_probs = np.exp(y_onnx_plate) / np.sum(np.exp(y_onnx_plate), axis=-1, keepdims=True)

    # 获取最高概率的字符索引
    index = np.argmax(plate_probs, axis=-1)

    # 解码字符索引为字符预测
    plate_no, new_index = decodePlate2(index[0])

    # 重新调整概率值，对应于解码后的字符
    prob = plate_probs[0, np.arange(plate_probs.shape[1]), index[0]][new_index]

    # 对颜色预测应用softmax
    # color_probs = np.exp(y_onnx_color) / np.sum(np.exp(y_onnx_color)) //二维数组
    color_probs = (np.exp(y_onnx_color) / np.sum(np.exp(y_onnx_color))).flatten() # 一维数组

    print("color_probs:", color_probs)
    # 获取颜色的最高置信度和索引
    color_index = np.argmax(color_probs)
    print("color_index:", color_index)
    color_conf = color_probs[color_index]

    # 检查颜色索引是否在范围内
    if color_index < len(plate_color_list):
    ...
    # 打印识别结果
    print("车牌颜色:", plate_color, "车牌号:", plate_no, "置信度:", prob.mean(axis=0), "颜色置信度:", color_conf, "颜色置信度:", color_probs)
...
def rec_plate(outputs, img0, session_rec):  # 识别车牌
    ...
    for output in outputs:
        ...
        # 输出详细结果
        plate_no, plate_color = get_plate_result2(roi_img, session_rec)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_path', type=str, default='/Users//Downloads/202312/20231205/330824000/', help='source')
