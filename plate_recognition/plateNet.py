...
class myNet_ocr_color(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.loc(x)
        x = self.newCnn(x)
        # print("----->x.shape:", x.shape) #torch.Size([1, 78, 1, 21])
        if self.export:
            conv = x.squeeze(2)  # b *512 * width
            conv = conv.transpose(2, 1)  # [w, b, c]
            if self.color_num:
                print("----yes export--->conv.shape:", conv.shape,"x_color:",x_color)
                return conv, x_color
            print("--->conv.shape:", conv.shape)  # torch.Size([1, 21, 78])
            return conv
        else:
            b, c, h, w = x.size()
            assert h == 1, "the height of conv must be 1"
            ...
            if self.color_num:
                print("---no export---->output.shape:", output.shape,"x_color:",x_color)
                return output, x_color
            print("--->output.shape:", output.shape) #orch.Size([21, 1, 78])
            return output
