...
def save_labelme_format(orgimg, dict_list, img_path, output_dir):
    """
    将检测结果保存为LabelMe格式的JSON文件
    ...
    labelme_data = {
        ...
        "imagePath": img_name,
        "imageData": None,
        "imageHeight": orgimg.shape[0],
        "imageWidth": orgimg.shape[1]
    }

    # 添加每个检测到的车牌作为标注
    for result in dict_list:  # 修正：直接遍历列表中的元素
        shape = {
            "label": str(result['class']),  # 类别标签 ('0' 或 '1')
            "points": [],  # 四个关键点
            "group_id": None,
            "shape_type": "polygon",
            "flags": {}
        ...
        for point in landmarks:
            shape["points"].append([float(point[0]), float(point[1])])
...
if __name__ == '__main__':
    """
    目的是用这个脚本把识别的四个关键点坐标转成LabelMe格式的标注文件
    """
    ...
    parser.add_argument('--image_path', type=str, default='imgs4/', help='source')
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--output', type=str, default='imgs3_result', help='source')
    ...
    # 创建无车牌图片的目录
    no_plate_dir = os.path.join(save_path, "no_plate")
    if not os.path.exists(no_plate_dir):
        os.mkdir(no_plate_dir)
    ...
    if not os.path.isfile(opt.image_path):  # 目录
        ...
        print(f"检测目录下{len(file_list)}张图片")
        for img_path in file_list:
            ...
            print(f"{opt.image_path}检测到{len(dict_list)}个车牌")

            if len(dict_list) > 0:  # 检测到车牌
                ori_img = draw_result(img, dict_list)
                # 保存LabelMe格式的标注文件
                save_labelme_format(img, dict_list, img_path, save_path)
                ...
                img_name = os.path.basename(img_path)
                no_plate_path = os.path.join(no_plate_dir, img_name)
                # 将图片移动到无车牌目录
                shutil.move(img_path, no_plate_path)
                print(f"未检测到车牌，已移动至: {no_plate_path}")
        ...
        # 保存LabelMe格式的标注文件
        save_labelme_format(img, dict_list, opt.image_path, save_path)
