...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/plate_detect.pt', help='weights path')
    ...
    model_stride_strings = ['stride_' + str(int(x)) for x in model.stride]
    print(f'-----model.stride:{model.stride},model_stride_strings:{model_stride_strings}')
...
# 测试
# python onnx_infer.py --detect_model weights/plate_detect-copy.onnx  --rec_model weights/plate_rec_color.onnx  --image_path imgs --output result_onnx

 # ```
 #   1. python ncnn_export.py --weights ./weights/plate_detect-copy.pt --img 640 --batch 1
 #   2. onnxsim weights/plate_detect.onnx weights/plate_detect.onnx --skip-shape-inference
 #   ```
