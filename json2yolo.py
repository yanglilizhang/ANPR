...
def order_points(pts):
    ...
    rect[1] = pts[np.argmin(diff)]  # 右上角点(x-y差值最小)
    rect[3] = pts[np.argmax(diff)]  # 左下角点(x-y差值最大)
...
def four_point_transform(image, pts):
    """将任意四边形区域变换为规则矩形图像，常用于矫正倾斜或变形的图像区域。"""
    rect = order_points(pts)
    # tl: 左上角点  tr: 右上角点 br: 右下角点 bl: 左下角点
    (tl, tr, br, bl) = rect
    # 计算宽度（取上下两边的最大值）
    ...
    # 通过计算四边形上下两边和左右两边的长度，确定目标矩形的宽度和高度
    # 定义变换后的目标坐标，将原始四边形映射到规则矩形
    dst = np.array([
        [0, 0],
        [maxWidth - 1, 0],
        [maxWidth - 1, maxHeight - 1],
        [0, maxHeight - 1]], dtype="float32")
    # 透视变换函数将图像从原始四边形区域变换到规则矩形区域
